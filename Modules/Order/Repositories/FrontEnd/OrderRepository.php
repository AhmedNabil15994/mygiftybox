...
class OrderRepository
{
    ...
    public function createOrderAddress($orderCreated, $address)
    {
        ...
        $request['mobile'] = str_replace(' ','',$address['mobile']);
         ...
         return $orderCreated->orderAddress()->create([
            ...
            'mobile' => str_replace(' ','',($address['calling_code'].$address['mobile'])) ?? optional(auth()->user())->mobile,
             'address' => $address['attributes'][41] ?? $address['address'],
             'block' => $address['attributes'][39] ?? $address['block'],
             'building' => $address['attributes'][40] ?? $address['building'],
             'street' => $address['attributes'][38] ?? $address['street'],
