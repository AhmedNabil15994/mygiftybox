...
<script>
    ...
    function dateRange(id, startName = 'from', endName = 'to',start = NaN,end = NaN) {
        $('#'+id).daterangepicker({
            startDate: start,
            endDate: end,
            ranges: {
                '{{__('apps::dashboard.general.date_range.all')}}': [NaN, NaN],
                '{{__('apps::dashboard.general.date_range.today')}}': [moment(), moment()],
                '{{__('apps::dashboard.general.date_range.yesterday')}}': [moment().subtract(1, 'days'), moment().subtract(1, 'days')],
                '{{__('apps::dashboard.general.date_range.7days')}}': [moment().subtract(6, 'days'), moment()],
                '{{__('apps::dashboard.general.date_range.30days')}}': [moment().subtract(29, 'days'), moment()],
        ...
        }, function(id,startName = 'from', endName = 'to',start = NaN,end = NaN){

            if ((isNaN(start) && isNaN(end)) || (start == null && end == null)) {

                $('#'+id+' span').html('{{__('apps::dashboard.general.date_range.all')}}');
                ...
                $('input[name="'+startName+'"]').val(start.format('YYYY-MM-DD'));
                $('input[name="'+endName+'"]').val(end.format('YYYY-MM-DD'));
            }
        });
    }
</script>
