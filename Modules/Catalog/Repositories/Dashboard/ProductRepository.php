...
class ProductRepository
{
    ...
    public function findByInternational_code($sku)
    {
    ...
    public function create($request)
    {
        ...
        try {
            $request['category_id'] = explode(',',$request['category_id']);
            ...
            if ($request->category_id) {
                ...
                if (!is_null($request['offer_price']) || !is_null($request['offer_percentage'])) {
...
//                    'image' => $request['v_images'][$key] ? path_without_domain($request['v_images'][$key]) : $model->image
    ...
    public function productOffer($model, $request)
    {
        if (!is_null($request['offer_price']) || !is_null($request['offer_percentage'])) {
            $data = [
                'status' => !is_null($request['offer_price']) || !is_null($request['offer_percentage']) ? true : false,
